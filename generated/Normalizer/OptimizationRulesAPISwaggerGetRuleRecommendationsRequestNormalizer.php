...
class OptimizationRulesAPISwaggerGetRuleRecommendationsRequestNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('ruleSubCategoryFilter') && null !== $data->getRuleSubCategoryFilter()) {
            $dataArray['ruleSubCategoryFilter'] = $this->normalizer->normalize($data->getRuleSubCategoryFilter(), 'json', $context);
        }
        if ($data->isInitialized('recommendationIdFilter') && null !== $data->getRecommendationIdFilter()) {
            $dataArray['recommendationIdFilter'] = $data->getRecommendationIdFilter();
        }
        if ($data->isInitialized('campaignIdListFilter') && null !== $data->getCampaignIdListFilter()) {
            $values = [];
            foreach ($data->getCampaignIdListFilter() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $dataArray['campaignIdListFilter'] = $values;
        }
        ...
        if ($data->isInitialized('ruleCategoryFilter') && null !== $data->getRuleCategoryFilter()) {
            $dataArray['ruleCategoryFilter'] = $this->normalizer->normalize($data->getRuleCategoryFilter(), 'json', $context);
        }
        foreach ($data as $key => $value_1) {
            if (preg_match('/.*/', (string) $key)) {
